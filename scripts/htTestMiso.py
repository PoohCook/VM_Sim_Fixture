...
class HtTestMiso():
    ...
    def run(self):
        self.__framework.start("HT Miso loopback")

        # setup mux for
        #       output HT:MC_MISO_TX       -->
        #       input  HT:MC_MISO_RX       <--
        self.__framework.fixtureSetMux(InputMux.HtMcMiso, OutputMux.HtIrMiso)
        ...
        # time.sleep(0.01)
        adcData = self.__framework.fixtureAdcReadData(channel=SenseChannel.SenseHtMcMiso, size=120)
        ...
        self.__framework.addResult(name="HT Miso Driver Voltages",
        ...
        self.__framework.pause()
        data_in2 = self.__framework.fixtureSerialRead(16)

        self.__framework.addResult(name="HT Passthrough off",
                                   result=(data_in == "[]") and (con_in == data_out) and (data_in2 == data_out2))


if __name__ == "__main__":
    parser = TestArguments("HtTestMiso")
    args = parser.parse_args()
    framework = TestFramework(args=args)

    def tests():
        test = HtTestMiso(framework)
        test.run()
