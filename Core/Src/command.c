...
static bool frame_data_read(CMD_FRAME* frame, CIRCULAR_DMA_BUFFER* circ){
    ...
    switch(frame->state){
        ...
        case CMD_STATE_DATA:
            ...
            if((frame->rx_len + 1) >= frame->frm_len){
...
static void cmd_send_response(COMMAND command, uint8_t* data, int length){
...
static void ser_send_with_ack(CMD_FRAME* frame, bool wait){
    if(!ser_send(frame->data, frame->frm_len-2, wait)){
        cmd_send_response(Nak, NULL, 0);
    } else{
        cmd_send_response(Ack,frame->data, frame->frm_len-2);
    }
}
...
static void frame_process(CMD_FRAME* frame){
    ...
    switch((COMMAND)frame->command){
        case Ping:
            cmd_send_response(Pong, frame->data, frame->frm_len-2);
            break;

        case VersionRead:
            FIXTURE_VERSION version = version_data();
            cmd_send_response(Ack, (uint8_t*)&version, sizeof(version));
            break;
        ...
        case SyncRead:
            ...
            cmd_send_response(Ack, (uint8_t*)&active, 1);
            break;
        ...
        case AdcRead:
            ...
            cmd_send_response(Ack, frame->data, rx_len * 2);
            break;
        ...
        case LedStatus:
            ...
            cmd_send_response(Ack, (uint8_t*)&led_status, 1);
            break;

        default:
            cmd_send_response(Nak, &(frame->command), 1);
    }
